...
exports.createCourse = async (req, res) => {
  try {
    ...
    const thumbnailImage = await fileUploadToCloudinary(
      thumbnail,
    ...
    const newCourse = await Course.create({
      ...
      status: status, // Ensure status is either Draft or Published
      instructions,
    });

    // Update User Schema for Instructor
...
//       if (updates.hasOwnProperty(key) && key !== "courseId") { // Ignore courseId in updates
...
//           }
//         } else {
//           // Avoid directly assigning undefined values
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        courseDetails,
        totalDuration,
        completeVideo: courseProgressCount?.completeVideo
...
exports.deleteCourse = async (req, res) => {
  try {
    ...
    const studentsEnrolled = course.studentsEnrollId
